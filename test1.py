...
Test'''
    ...
    def services_screen(self, width, height):
        ...
        self.bg_photo = ImageTk.PhotoImage(bg_image)
        self.bg_photo_label = tk.Label(self.root, image=self.bg_photo, bd=0, highlightthickness=0)
        self.bg_photo_label.place(x=0, y=header_image_height)

        self.main_frame = tk.Frame(self.root)
        self.main_frame.place(x=0, y=header_image_height, width=width, height=height-header_image_height)

        # Create canvas and scrollbar directly on the root window
        self.canvas = tk.Canvas(self.root, highlightthickness=0, bd=0)
        self.canvas.place(in_=self.main_frame, relx=0, rely=0, relwidth=1, relheight=1)

        self.scrollbar = tk.Scrollbar(self.root, orient=tk.VERTICAL, command=self.canvas.yview)
        self.scrollbar.place(in_=self.main_frame, relx=1, y=(3*header_image_height+14), relheight=1, anchor=tk.E)

        self.canvas.configure(yscrollcommand=self.scrollbar.set)
        self.canvas.bind("<Configure>", lambda e: self.canvas.configure(scrollregion=self.canvas.bbox("all")))
        self.bg_photo_label.lift()
        self.scrollbar.lift()
